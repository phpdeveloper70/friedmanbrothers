...
function choose_image_size($path, $type){
	$types["thumb"] = "_tn";
	$types["small"] = "";
	$types["medium"] = "_01";
	$types["large"] = "_lg";
	...
	if(count($parts) > 3){
		$part_one = "";
		$part_two = $parts[count($parts)-1];
		for($i=0; $i<count($parts)-1; $i++){
			$part_one .= $parts[$i].".";
		}
		$part_one = substr($part_one, 0, strlen($part_one) -1);
		$parts = array($part_one,$part_two);
	}
	...
	return $parts[0].$types[$type].".".$parts[1];
}
?>
